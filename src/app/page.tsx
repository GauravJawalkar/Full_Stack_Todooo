    ...
    <section >
      ...
      <div>
        <div className="flex items-center justify-center gap-5 my-5">
          ...
          <button onClick={() => { modal ? setModal(false) : ""; response(); }} className="bg-[#1a1a1a] px-4 py-2 rounded ring-1 ring-gray-700 hover:bg-black hover:ring-gray-500 transition-all ease-linear duration-200">All Tasks</button>
          <button onClick={() => {
            completedTasks();
            setModal(false)
      ...
      <div className={`grid grid-cols-4 ${data.length === 0 ? "grid-cols-none" : ""} max-sm:grid-cols-1 gap-10 p-10`}>
        {
          data.length > 0 ? data.map(({ _id, title, deadline }) => {
            ...
            <div className={`animate-pulse transition-all ease-linear duration-200 ${modal === true ? "hidden" : ""} text-center`}>
